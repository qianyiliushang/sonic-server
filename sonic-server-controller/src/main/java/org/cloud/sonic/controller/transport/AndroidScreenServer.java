...
@Component
@Slf4j
@ServerEndpoint(value = "/android/screen/{agentKey}/{udId}/{token}/{agentIp}/{agentPort}", configurator = WsEndpointConfigure.class)
public class AndroidScreenServer {
    @OnOpen
    public void onOpen(Session session, @PathParam("agentKey") String agentKey, @PathParam("udId") String udId,
        ...
        String agentUri = String.format("ws://%s:%s/websockets/android/screen/%s/%s/%s", agentIp, agentPort, agentKey, udId, token);
    ...
    public void onMessage(Session session, byte[] message) {
        Session client = BytesTool.sessionList.getClientSessionByBrowserSessionId(session.getId());
        log.info("screen receive byte message");
        BytesTool.send(client, message);
    }
