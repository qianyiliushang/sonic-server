...
@Component
@Slf4j
@ServerEndpoint(value = "/android/{agentKey}/{udId}/{token}/{agentIp}/{agentPort}", configurator = WsEndpointConfigure.class)
public class AndroidWSServer {
    ...
    public void onOpen(Session session, @PathParam("agentKey") String agentKey, @PathParam("udId") String udId,
        ...
        String agentUri = String.format("ws://%s:%s/websockets/android/%s/%s/%s", agentIp, agentPort, agentKey, udId, token);
